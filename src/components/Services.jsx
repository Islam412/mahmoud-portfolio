...
const Services = () => {
  ...
  return (
    <section id="services" className="py-20 bg-dark-200" ref={ref}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {servicesItems.map((service, index) => {
            ...
            return (
              ...
              >
                <div className={`w-16 h-16 rounded-xl bg-gradient-to-r ${colorClass} flex items-center justify-center mb-6 group-hover:scale-110 transition-transform`}>
                  <IconComponent className="w-8 h-8 text-white" />
                </div>
                <h3 className="text-2xl font-bold mb-2 text-white">{service.title}</h3>
                <p className="text-sm text-accent-primary mb-3">{service.titleEn}</p>
